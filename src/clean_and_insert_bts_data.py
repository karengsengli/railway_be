    ...
    async with async_session() as session:
        try:
            ...
            result = await session.execute(text("""
                INSERT INTO regions (name, code, country, timezone, created_at, updated_at)
                VALUES ('Bangkok', 'BKK', 'Thailand', 'Asia/Bangkok', NOW(), NOW())
                RETURNING id
            """))
            ...
            # Create BTS company
            result = await session.execute(text("""
                INSERT INTO train_companies (name, code, region_id, website, is_active, created_at, updated_at)
                VALUES ('Bangkok Mass Transit System', 'BTS', :region_id, 'https://www.bts.co.th', true, NOW(), NOW())
                RETURNING id
            ...
            result = await session.execute(text("""
                INSERT INTO train_companies (name, code, region_id, website, is_active, created_at, updated_at)
                VALUES ('Bangkok Metro', 'BMCL', :region_id, 'https://www.bangkokmetro.co.th', true, NOW(), NOW())
                RETURNING id
            """), {"region_id": bangkok_region_id})
            ...
            # Create train lines
            lines_data = [
                ("Green Line (Sukhumvit)", "GREEN_SUK", "Green", bts_company_id),
                ("Green Line (Silom)", "GREEN_SIL", "Green", bts_company_id),
                ("Gold Line", "GOLD", "Gold", bts_company_id),
                ("Pink Line", "PINK", "Pink", bmcl_company_id),
                ("Yellow Line", "YELLOW", "Yellow", bmcl_company_id),
            ]

            line_ids = {}
            for name, code, color, company_id in lines_data:
                result = await session.execute(text("""
                    INSERT INTO train_lines (name, code, color, company_id, is_active, created_at, updated_at)
                    VALUES (:name, :code, :color, :company_id, true, NOW(), NOW())
                    RETURNING id
                """), {"name": name, "code": code, "color": color, "company_id": company_id})
            ...
            for line_name, stations_data in BTS_STATION_DATA.items():
                ...
                for order, (code, name, latitude, longitude) in enumerate(stations_data, 1):
                    await session.execute(text("""
                        INSERT INTO stations (name, code, latitude, longitude, line_id, is_active, has_elevator, has_escalator, has_restroom, created_at, updated_at)
                        VALUES (:name, :code, :latitude, :longitude, :line_id, true, true, true, true, NOW(), NOW())
                    """), {
                        "name": name,
                        "code": code,
                        "latitude": latitude,
            ...
            for line_name, stations_data in BTS_STATION_DATA.items():
                ...
                for order, (code, name, latitude, longitude) in enumerate(stations_data, 1):
                    # Get station ID
                    result = await session.execute(text("""
                        SELECT id FROM stations WHERE code = :code AND line_id = :line_id
                    """), {"code": code, "line_id": line_id})
                    station_id = result.scalar()

                    if station_id:
                        await session.execute(text("""
                            INSERT INTO line_station_orders (line_id, station_id, order_position, created_at, updated_at)
                            VALUES (:line_id, :station_id, :order_position, NOW(), NOW())
                        """), {
                            "line_id": line_id,
                ...
                for i in range(len(ordered_stations) - 1):
                    ...
                    for from_id, to_id in [(from_station.id, to_station.id), (to_station.id, from_station.id)]:
                        await session.execute(text("""
                            ...
                            (line_id, from_station_id, to_station_id, distance_km, duration_minutes, created_at, updated_at)
                            VALUES (:line_id, :from_station_id, :to_station_id, :distance_km, :duration_minutes, NOW(), NOW())
                        """), {
                            "line_id": line_id,
            ...
            for station_name, description in interchange_data:
                ...
                if station:
                    await session.execute(text("""
                        INSERT INTO interchange_points (station_id, interchange_name, description, created_at, updated_at)
                        VALUES (:station_id, :interchange_name, :description, NOW(), NOW())
                    """), {
